...
ContextProviderInProcess::ContextProviderInProcess()
    : destroyed_(false)
    /// M: check if current has hardware compositor.
    ///   in case, create this CotnextProviderInProcess must included a hardware compositor.
    //    so default is true
    , has_hardware_compositor_(true) {
...
/// M: @{
bool ContextProviderInProcess::HasHardwareCompositor() {
  base::AutoLock lock(compositor_check_lock_);
  return has_hardware_compositor_;
}

void ContextProviderInProcess::SetHasHardwareCompositor(bool has_hardware_compositor) {
  base::AutoLock lock(compositor_check_lock_);
  has_hardware_compositor_ = has_hardware_compositor;
}
/// @}
}  // namespace gpu
}  // namespace webkit
