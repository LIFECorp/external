...
/// M: added by Willy @{
bool WebExternalTextureLayerImpl::skipAcquireTexture() {
    LOG(INFO) << "[WebExternalTextureLayerImpl::skipAcquireTexture] type=[" << client_->type()<<"]";
    if (client_->type() == WebKit::WebExternalTextureLayerClient::TEXTURE_WEBGL)
        return true;
    return false;
}
/// @}
