...
void SynchronousCompositorFactoryImpl::CompositorInitializedHardwareDraw(
  ...
  /// M: in case no hardware compositor, should not get gl context.@{
  if (offscreen_context_for_main_thread_.get())
    offscreen_context_for_main_thread_->SetHasHardwareCompositor(true);
  /// @}
}
...
void SynchronousCompositorFactoryImpl::CompositorReleasedHardwareDraw(
  ...
  bool should_release_resources = false;
  {
    ...
    /// M: in case no hardware compositor, should not get gl context.@{
    if (num_hardware_compositors_ == 0u) {
      should_release_resources = true;
      if (offscreen_context_for_main_thread_.get())
        offscreen_context_for_main_thread_->SetHasHardwareCompositor(false);
    }
    /// @}
  }
...
/// M: for ALPS01298519, don't request drawGL when surface is not ready @{
bool SynchronousCompositorImpl::CanRequestDrawGL()
{
    if (output_surface_)
      return output_surface_->CanBeginFrame();
    return false;
}
/// @}
