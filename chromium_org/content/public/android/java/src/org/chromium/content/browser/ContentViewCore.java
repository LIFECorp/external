...
@JNINamespace("content")
    ...
    /// M: for CTS fail.
    private boolean mIsCTSApp = false;
    /**
    ...
    public void onSizeChanged(int wPix, int hPix, int owPix, int ohPix) {
        ...
        /// M: When height or width is zero, just return to prevent layout process
        if (mIsCTSApp
            && ((wPix == 0 && owPix == 0) || (hPix == 0 && ohPix == 0))) {
    ...
    private void onSelectionBoundsChanged(Rect anchorRectDip, int anchorDir, Rect focusRectDip,
        ...
        if (x1 != x2 || y1 != y2 ||
            ...
            if (!inputBounds.isEmpty()) {
                // expand the right and bottom 1px for contains
                inputBounds.set(inputBounds.left, inputBounds.top, inputBounds.right + 1, inputBounds.bottom + 1);
                if (isAnchorFirst) {
                    startIsOut = !inputBounds.contains(x1, y1);
                    endIsOut = !inputBounds.contains(x2, y2);
                } else {
                    startIsOut = !inputBounds.contains(x2, y2);
                    endIsOut = !inputBounds.contains(x1, y1);
                }
            }
