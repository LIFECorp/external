...
void FindHelper::FindAllAsync(const string16& search_string) {
  ...
  LOG(WARNING) << "FindHelper::FindAllAsync searching...id: "
               << current_request_id_;
...
void FindHelper::HandleFindReply(int request_id,
                                   ...
                                   int active_ordinal,
                                   bool finished) {
  LOG(WARNING) << "FindHelper::HandleFindReply "
...
void FindHelper::FindNext(bool forward) {
  ...
  LOG(WARNING) << "FindHelper::FindNext searching...id: "
               << current_request_id_;
  web_contents()->GetRenderViewHost()->Find(current_request_id_,
                                            last_search_string_,
                                            options);
...
void FindHelper::NotifyResults(int active_ordinal,
                                 int match_count,
                                 bool finished) {
  LOG(WARNING) << "FindHelper::NotifyResults "
               << active_ordinal << ", " << match_count << ", " << finished;
