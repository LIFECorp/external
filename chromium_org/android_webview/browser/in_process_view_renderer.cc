...
bool InProcessViewRenderer::OnDraw(jobject java_canvas,
  ...
  if (is_hardware_canvas && attached_to_window_ && HardwareEnabled()) {
    ...
    /// M: walkaround for suspend drawing which cause black screen
    ///  root cause is HWUI's dirty region was turned off
    return compositor_ && compositor_->CanRequestDrawGL() && client_->RequestDrawGL(java_canvas);
  }
...
void InProcessViewRenderer::OnAttachedToWindow(int width, int height) {
  /// M: Add debug log @ {
  LOG(INFO) << "[InProcessViewRenderer::OnAttachedToWindow()]["
    << this << "][" << getpid() << "][" << gettid() << "]";
  /// @ }
  ...
  /// M: Store the renderer early since it's attached to window @ {
  manager_key_ = g_view_renderer_manager.Get().DidDrawGL(manager_key_, this);
  /// @ }
}
...
void InProcessViewRenderer::OnDetachedFromWindow() {
  /// M: Add debug log @ {
  LOG(INFO) << "[InProcessViewRenderer::OnDetachedFromWindow()]["
    << this << "][" << getpid() << "][" << gettid() << "]";
  /// @ }
