...
void TileManager::AssignGpuMemoryToTiles(
  ...
  /// M: for memory usage dump
  dumpMemoryStats();
}

/// M: for memory usage dump @{
void TileManager::dumpMemoryStats() {

  base::Time now = base::Time::Now();
  if ((now - last_dump_time_).InSecondsF() < 1)
    return;

  bool need_push = false;
  if (memory_stats_history_.size()) {
    if (memory_stats_history_[memory_stats_history_.size() - 1].second != memory_stats_from_last_assign_)
      need_push = true;
  } else
    need_push = true;

  if (need_push)
    memory_stats_history_.push_back(std::make_pair(now, memory_stats_from_last_assign_));

  if (memory_stats_history_.size() == 5) {
    LOG(INFO) << "[TileManager] +++++++++++++ Memory History +++++++++++++";
    LOG(INFO) << "[TileManager] [ ] [  Budget  ] [ Allocated] [ Unrelease] [   Over   ]";
    for (size_t i = 0; i < memory_stats_history_.size(); i++) {
      std::string str;
      base::StringAppendF(&str, "[%d] [%10d] [%10d] [%10d] [%10d] at %5.4f s before"
        , i
        , memory_stats_history_[i].second.total_budget_in_bytes, memory_stats_history_[i].second.bytes_allocated
        , memory_stats_history_[i].second.bytes_unreleasable, memory_stats_history_[i].second.bytes_over
        , (now - memory_stats_history_[i].first).InSecondsF()
        );
      LOG(INFO) << "[TileManager] " << str;
    }
    LOG(INFO) << "[TileManager] ------------- Memory History -------------";
    memory_stats_history_.clear();
  }
  last_dump_time_ = now;
}
/// @}
