...
void WorkerPool::Inner::SetTaskGraph(TaskGraph* graph) {
...

  {
    ...
    for (GraphNodeMap::const_iterator it = pending_tasks_.begin();
         ...
         ++it) {
      LOG(INFO) << "[SetTaskGraph] [" << this << "][" << getpid() << "][" << gettid()
                    << "] cancel tasks pending=[" << it->first << "] graph=[" << it->second << "]";
...
void WorkerPool::Inner::Run() {
  ...
  while (true) {
    ...
    /// M: ALPS01270059 check the task if in pending_tasks.
    ///     if not may mean, the Graph Node was deleted @{
    internal::WorkerPoolTask* t = 0;
    while ((t = ready_to_run_tasks_.top()->task())) {
      if (!pending_tasks_.get(t)) {
        // task not found from pending_tasks.
        LOG(ERROR) << "[WorkerPool::Inner::Run] [" << this << "][" << getpid() << "][" << gettid()
                    << "] task=" << t << " is not in pending_tasks!!!";
    ...
    if (node) {
      for (internal::GraphNode::Vector::const_iterator it =
        ...
        if (!dependent_node->num_dependencies()) {
          /// M: check if the dependent_node if in pending_tasks @{
          if (!pending_tasks_.get(dependent_node->task())) {
            LOG(ERROR) << "[Inner::Run] [" << this << "][" << getpid() << "]["
                        << gettid() << "] dependentNode=[" << dependent_node << "] is not in pending_tasks";
            continue;
          }
          /// @}
