 ...
 public:
  ...
  struct Entry {
    ...
    Entry(const Entry& entry)
        : total_budget_in_bytes(entry.total_budget_in_bytes),
          bytes_allocated(entry.bytes_allocated),
          bytes_unreleasable(entry.bytes_unreleasable),
          bytes_over(entry.bytes_over) {}

    bool operator==(Entry& entry) {
      if (total_budget_in_bytes == entry.total_budget_in_bytes
          && bytes_allocated == entry.bytes_allocated
          && bytes_unreleasable == entry.bytes_unreleasable
          && bytes_over == entry.bytes_over)
        return true;
