...
#define DUMP_RENDER_TREE_THRESHOLD 450
...
LayerTreeHost::LayerTreeHost(LayerTreeHostClient* client,
  ...
  /// M: add log for ALPS01278872 @{
  is_user_load_ = base::SysInfo::IsUserLoad();
  /// @}
}
...
/// M: traverse layer hierarchy for ALPS01278872 @{
bool LayerTreeHost::traverseLayerTree(Layer* layer, int indent)
{
  if (indent > DUMP_RENDER_TREE_THRESHOLD) {
    LOG(ERROR) << "[LayerTreeHost::traverseLayerTree] indent=[" << indent << "] layer=[" << layer
            << "] layer hierarchy exceed threshold";
    return false;
  }
  const cc::LayerList& childHost = layer->children();
  for (cc::LayerList::const_iterator iter = childHost.begin(); iter != childHost.end(); ++iter) {
    if (!traverseLayerTree(iter->get(), (indent + 1)))
      return false;
  }
  return true;
}
...
bool LayerTreeHost::UpdateLayers(ResourceUpdateQueue* queue,
  ...
  /// M: check tree hierarchy for ALPS01278872 @{
  ///   Note: This only for eng load, call dumpRenderingTree will dump render layer and crash.
  if (!is_user_load_ && !traverseLayerTree(root_layer_.get())) {
    LOG(ERROR) << "[UpdateLayers] [" << this << "][" << getpid() << "][" << gettid() << "] Layer tree has problem !!!";
    // Note: this function will call CRASH
    client_->dumpRenderingTree();
  }
  /// @}
