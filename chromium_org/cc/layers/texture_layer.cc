...
void TextureLayer::SetTextureId(unsigned id) {
  ...
  if (texture_id_ && layer_tree_host()) {
    LOG(INFO) << "[TextureLayer::SetTextureId] [" << this << "][" << getpid() << "][" << gettid()
        << "] ready to acquireLayerTextures";
...
void TextureLayer::WillModifyTexture() {
  if (layer_tree_host() && (DrawsContent() || content_committed_)) {
    LOG(INFO) << "[TextureLayer::WillModifyTexture] [" << this << "][" << getpid() << "][" << gettid()
        << "] ready to acquireLayerTextures useMailbox="<<uses_mailbox_<<"]["<<client_->skipAcquireTexture()<<"]";
    if (!uses_mailbox_ && client_->skipAcquireTexture())
    layer_tree_host()->AcquireLayerTextures();
    content_committed_ = false;
  }
}
...
void TextureLayer::SetLayerTreeHost(LayerTreeHost* host) {
  ...
  if (layer_tree_host()) {
    if (texture_id_) {
      LOG(INFO) << "[TextureLayer::SetLayerTreeHost] [" << this << "][" << getpid() << "][" << gettid()
        << "] ready to acquireLayerTextures";
      layer_tree_host()->AcquireLayerTextures();
    }
