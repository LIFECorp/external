...
/*#if PLATFORM(MAC) || (PLATFORM(CHROMIUM) && OS(DARWIN))
    ...
    } else if (event->type() == eventNames().mouseupEvent && event->isMouseEvent() && !toRenderBox(element->renderer())->canBeScrolledAndHasScrollableArea()) {//static_cast<MouseEvent*>(event)->button() == LeftButton && element->document()->frame()->eventHandler()->autoscrollRenderer() != element->renderer()) {
