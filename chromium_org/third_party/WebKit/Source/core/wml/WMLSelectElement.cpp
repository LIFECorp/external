...
/// M: @{
bool WMLSelectElement::childShouldCreateRenderer(const NodeRenderingContext& childContext) const
{
    if (!m_data.usesMenuList())
        return childContext.node()->hasTagName(WMLNames::optionTag) || isWMLOptGroupElement(childContext.node());
    return false;
}
/// @}
